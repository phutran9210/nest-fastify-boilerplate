...
hiện có và **cập nhật toàn bộ** slash commands + `CLAUDE.md` cho khớp.
...
```ts
// modules/users/services/users.service.ts  — không còn this.prisma
import { Injectable, NotFoundException } from '@nestjs/common';
import type { User } from '../../../generated/prisma/client';
import { type CreateUserData, type UpdateUserData, UserRepository } from '../repositories/user.repository';
...
**Ghi chú thiết kế:** port trả `User` (type model Prisma — chỉ là shape hàng dữ liệu, không phải runtime
coupling). Input dùng type module-local (`CreateUserData`/`UpdateUserData`) để KHÔNG leak
`Prisma.UserCreateInput`. Muốn tách hẳn domain entity + mapper là follow-up, không làm bây giờ (YAGNI).
...
**Nguyên tắc:** sau mỗi lần di chuyển, sửa import của file vừa chuyển VÀ mọi file import tới nó. Dùng
`pnpm build` (tsc) để bắt import sai — build phải sạch.
...
- **create-module.md**: cây file mới; sinh thêm `repositories/<feature>.repository.ts` (port) +
  ...
  controller/service/dto vào subfolder.
...
- Docs (commands + CLAUDE.md) khớp cấu trúc mới, không còn mô tả "flat".
