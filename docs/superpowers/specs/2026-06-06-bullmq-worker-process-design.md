...
| Bảo vệ Bull Board | **Basic Auth** qua env, dùng **custom Nest middleware tự viết** (KHÔNG dùng `fastify-basic-auth` — deprecated; `@fastify/basic-auth` v6 là plugin Fastify, không khớp connect-middleware) |
...
**`src/worker.module.ts`** — root module worker process (`implements NestModule`):
- `imports`:
  ...
  - `BullBoardModule.forRoot({ route: '/admin/queues', adapter: FastifyAdapter })` từ `@bull-board/nestjs` + `@bull-board/fastify` (KHÔNG truyền option `middleware` — auth làm bằng Nest middleware riêng bên dưới).
...
- `configure(consumer)`: apply `BullBoardAuthMiddleware` cho route `/admin/queues*`.
...
**`src/common/middleware/bull-board-auth.middleware.ts`** — Basic Auth tự viết (`implements NestMiddleware`):
- Đọc header `Authorization: Basic <base64>` trên **raw req** (Nest+Fastify middleware chạy trên raw req/res — đã verify qua doc NestJS).
- So khớp `BULLBOARD_USER`/`BULLBOARD_PASSWORD` (qua `ConfigService`). Sai/thiếu → `res.statusCode = 401`, set header `WWW-Authenticate: Basic realm="Bull Board"`, kết thúc response; đúng → `next()`.
- KHÔNG phụ thuộc package auth ngoài.

> **Điểm cần verify ở bước plan/impl:** Bull Board (Fastify adapter) đăng ký route qua Fastify plugin. Phải xác nhận Nest middleware (`consumer.apply().forRoutes('/admin/queues*')`) thực sự chặn được các route do plugin tạo. Nếu không, fallback: đăng ký `onRequest` hook của Fastify trong `main.worker.ts` (`app.getHttpAdapter().getInstance().addHook('onRequest', …)`) giới hạn theo path prefix. Chọn cơ chế nào **chặn được route Bull Board** thì dùng.
...
```
> Dev: `BULLBOARD_PASSWORD` không set → middleware coi như `'admin'` (chỉ khi **không** phải production). Prod: schema fail-fast nếu thiếu → app worker không boot với credential mặc định. (Lưu ý: `.superRefine` đổi kiểu trả về của schema — kiểm tra `validateEnv` / `z.infer` vẫn đúng ở bước impl.)
...
- **Unit** (`test/unit/`, mirror src, alias import, mock PORT — theo convention):
  ...
  - Thêm `test/unit/common/middleware/bull-board-auth.middleware.spec.ts`: thiếu header → 401 + `WWW-Authenticate`; sai creds → 401; đúng creds → gọi `next()`. (Mock `ConfigService`, `req`/`res`.)
- **E2E — bắt buộc trong scope này** (điểm tích hợp rủi ro nhất): `test/e2e/worker-bull-board.e2e-spec.ts` bootstrap `WorkerModule` (Fastify) và assert:
  ...
  Đây cũng là chỗ kiểm chứng cơ chế chặn route Bull Board (middleware vs Fastify hook) hoạt động thật.
